...
app.get("/registerOptions", (req, res) => {
...
app.get("/registerDentist", (req, res) => {
    ...
    !req.query.type ? res.redirect("/login") : res.render("registerDentist",{registered:registered});
...
app.get("/registerPatient", (req, res) => {
    userType = req.query.type;
    //check if register page accessed from login to get user type
    !req.query.type ? res.redirect("/login") : res.render("registerPatient",{registered:registered});
...
app.post("/registerDentist", (req, res) => {
    ...
    User.register(new User({username: req.body.username, userType : userType, email : req.body.email}), req.body.password, function(err, user){
        if (err) {
            console.log(err)
            //already registered
            registered = "username already exists!";
            return res.render("registerDentist",{registered : registered});
        }
...
app.post("/registerPatient", (req, res) => {
    User.register(new User({username: req.body.username, userType : userType,email : req.body.email}), req.body.password, function(err, user){
        if (err) {
            ...
            //already registered
            registered = "username already exists!";
            return res.render("registerPatient",{registered : registered});
        }
        passport.authenticate("local")(req, res, function(){
            res.redirect("/");
        });
